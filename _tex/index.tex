...
\makeatother
\hypersetup{
  pdftitle={RaceIndyCar: Analysis of motorsport qualifying with a novel Python package},
  pdfauthor={Anonymous},
...
\title{\bf RaceIndyCar: Analysis of motorsport qualifying with a novel
Python package}
\author{
Anonymous\\
{[}Institution Name{]}\\
...
\section{RaceIndyCar}\label{raceindycar}

RaceIndyCar is a Python pip-installable package to obtain and analyze
IndyCar data. By scraping information from the official
\href{https://www.indycar.com/}{IndyCar} website, we can examine all
weekend events and their corresponding sessions.

The specific information we have available depends on the session and
release date. For example, not all races have the same number of
practice sessions. Additionally, lap-by-lap data is only available for
2013 and beyond (with all data being from 1996-Present).
...
itself). We use this idea of events and sessions in our implementation:
...
\NormalTok{raceindycar.enable\_cache(}
\NormalTok{    cache\_dir}\OperatorTok{=}\StringTok{".cache/raceindycar"}\NormalTok{,}
\NormalTok{    cache\_format}\OperatorTok{=}\StringTok{"pickle"}\NormalTok{,}
\NormalTok{)}
...
\CommentTok{\# Get a specific session from an event.}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{det25\_race }\OperatorTok{=}\NormalTok{ raceindycar.get\_session(}
    \DecValTok{2025}\NormalTok{,}
    \StringTok{"Detroit Grand Prix"}\NormalTok{,}
\NormalTok{    session}\OperatorTok{=}\StringTok{"R"}\NormalTok{,}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
every section of an IndyCar track. Lap results from a given session are
scraped from PDFs that are around 500 pages long, so loading laps can
take anywhere from 30--100 seconds.
...
\subsection{Package Design and Usage}\label{package-design-and-usage}
...
\NormalTok{fig, ax }\OperatorTok{=}\NormalTok{ plotting.plot\_position(}
\NormalTok{    det25\_race,}
\NormalTok{    drivers}\OperatorTok{=}\NormalTok{drivers,}
\NormalTok{)}
\NormalTok{fig.set\_size\_inches(}\DecValTok{4}\NormalTok{, }\FloatTok{2.5}\NormalTok{)}

\CommentTok{\# Lap time line chart}
    \CommentTok{\# fig, ax = plotting.plot\_lap\_times(}
    \CommentTok{\#    det25\_race,}
    \CommentTok{\#    drivers=drivers,}
    \CommentTok{\#)}
...
\NormalTok{det26\_race }\OperatorTok{=}\NormalTok{ raceindycar.get\_session(}
...
\CommentTok{\# Histogram of positions gained/lost from start to finish}
    \CommentTok{\#fig, ax = plotting.plot\_position\_gain(}
    \CommentTok{\#    combined\_results}
    \CommentTok{\#)}

\CommentTok{\# Scatter of a results metric vs qualifying position, with trend line}
    \CommentTok{\#fig, ax = plotting.plot\_metric\_vs\_qualifying(}
    \CommentTok{\#    combined\_results,}
    \CommentTok{\#    metric\_col="PointsEarned",}
    \CommentTok{\#)}
...
\begin{center}
\pandocbounded{\includegraphics[keepaspectratio]{index_files/figure-pdf/cell-7-output-1.pdf}}
\end{center}
...
For example, \texttt{plot\_qualifying\_vs\_finish} is shown here.
...
for \href{https://www.indycar.com/}{IndyCar}.

The second stage stores Pickle/CSV files that contain information from
loading a race.
...
\CommentTok{\# Enable Cache with specified directory.}
\CommentTok{\# Use either "pickle" or "csv" format.}
\NormalTok{raceindycar.enable\_cache(}
\NormalTok{    cache\_dir}\OperatorTok{=}\StringTok{".cache/raceindycar"}\NormalTok{,}
\NormalTok{    cache\_format}\OperatorTok{=}\StringTok{"pickle"}\NormalTok{,}
\NormalTok{)}
...
\CommentTok{\# Temporarily bypass the cache.}
\ControlFlowTok{with}\NormalTok{ Cache.disabled():}
\NormalTok{    ...}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{Summary and Application}\label{summary-and-application}
...
\section{Quantifying the Importance of
Qualifying}\label{quantifying-the-importance-of-qualifying}
...
Figure~\ref{fig-quali-eda} shows that in F1, for the high starting
positions, has a higher average finish position than the other two
series. However, this advantage compared to IndyCar ends at around
position 7. NASCAR's curve is flatter compared to the other two series,
indicating that average finish positions for many starting positions are
similar.
...
In addition, each NASCAR driver performs around 6 times as many passes
as an F1 driver and over double the number for IndyCar drivers.
...
NASCAR), confirming grid position is a faithful proxy for actual
race-start position across all three series. Using Lap 1 position
instead of qualifying position as the predictor yields even larger
partial R² values --- 0.257 (F1), 0.087 (IndyCar), 0.048 (NASCAR) ---
suggesting the true track-position effect is, if anything, slightly
understated by using the qualifying result alone rather than the
position a driver actually starts from once any grid penalties are
applied.

\section{Discussion}\label{discussion}

Qualifying proves to have statistically significant predictor of finish
position, adjusting for race performance. However, this effect decreases
in magnitude from F1 to IndyCar to NASCAR.
...
market. If they enjoy watching qualifying and with that, a larger time
investment in the sport, then perhaps F1 is the series to watch for
them, as it has a larger effect on race day results than IndyCar or
NASCAR. On the other hand, if they only want to watch an exciting race,
then IndyCar and/or NASCAR is the way to go because there is more
variance in finish position from start position, meaning there is more
passing and more excitement for the viewer on a lap-by-lap basis. This
is supported by the average number of passes for F1, IndyCar, and NASCAR
races, which are 5, 12, and 29, respectively.
...
performance because it has a larger effect on race results.

F1 drivers should be super aggressive on lap one. The difference in the
predictive value for qualifying position and lap one position is 8.7\%,
whilst only being 1.8\% and 0.6\% for IndyCar and NASCAR. These drivers
can take a more relaxed approach to the first lap, as it is not
devastating to informing race results.
